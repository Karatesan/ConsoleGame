...
public final class DropVerb implements Verb {
    ...
    @Override public Check validateStructural(VerbContext c) {
        return c.inv.arg(0) == null && !c.acceptsPipedMaterial() ? Check.invalid("drop what?", null) : Check.ok();
    }
    @Override public ExitCode execute(VerbContext c) {
        Item item = c.itemFromMaterialOrArg(0);
        if (item == null || !c.thrall.inventory().remove(item)) { c.say("not carrying that"); return ExitCode.BLOCKED; }
