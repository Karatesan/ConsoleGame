...
public final class InspectVerb extends FreeVerb {
    ...
    @Override public ExitCode execute(VerbContext c) {
        ...
        if (address == null) { c.say("inspect what?"); return ExitCode.INVALID; }
        Resolved resolved = VerbHelpers.resolve(c, address);
        if (resolved == null) { c.say("cannot perceive " + address); return ExitCode.INVALID; }
        switch (resolved) {
    ...
    private void inspectEntity(VerbContext c, Entity entity) {
        ...
        if (entity instanceof Actor actor && actor.mainHand() != null) output.append("\n  Holding: ").append(actor.mainHand().name());
        if (entity.readied() != null && !entity.isReadiedSpent()) output.append("\n  READIED: ")
                .append(entity.readied().description()).append(" (").append(entity.readied().damage()).append(" dmg)");
        for (BodyPart part : BodyPart.values()) output.append(String.format("%n  %-6s %3d%%  x%.1f", part.path,
                Math.max(5, 70 + part.hitMod - entity.evasion()), part.damageMult));
        c.say(output.toString());
    }

    private void inspectItem(VerbContext c, Item item) {
        c.say(item == null ? "nothing there" : item.name() + " — tags " + item.tags()
