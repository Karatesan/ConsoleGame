...
//        switch (e) {
//            case GameEvent.Narrative n -> put(n.text());
//            case GameEvent.Audit a -> put(a.text());
//            case GameEvent.Redraw r -> dirty = true;
//
//            case GameEvent.Rejected r -> {
//                put("x " + r.code() + " — " + r.reason());
//                if (r.hint() != null) put("    " + r.hint());
//                put("    0 AP spent. Line not counted.");
//            }
//            case GameEvent.LineStart s ->
//                    put("> " + s.raw() + "   [alloc " + s.allocation() + " AP, tax " + s.tax()
//                            + ", " + (s.chain() ? "CHAIN" : "SINGLE") + "]");
//
//            case GameEvent.StageResult s ->
//                    put(String.format("  [%d/%d] %-38s %-12s (%d AP)  AP %d",
//                            s.index(), s.total(), s.render(), s.code(), s.charged(), Math.max(0, s.apLeft())));
//
//            case GameEvent.StageSkipped s ->
//                    put(String.format("  [%d]   %-38s SKIPPED      (%s)", s.index(), s.render(), s.why()));
//
//            case GameEvent.LineComplete c -> {
//                put(String.format("  LINE COMPLETE — %d AP charged, %d tax, %d returned unused. AP %d.",
//                        c.charged(), c.tax(), c.returnedUnused(), c.apLeft()));
//                dirty = true;
//            }
//            case GameEvent.LineBroke b -> {
//                put("x BREAK — " + b.reason());
//                put(String.format("    Forfeited: %d AP allocation + %d AP penalty = %d AP.",
//                        b.allocation(), b.penalty(), b.forfeited()));
//                if (b.hint() != null) put("    Tip: " + b.hint());
//                dirty = true;
//            }
//            case GameEvent.InterruptFired i ->
//                    put("!! INTERRUPT — " + i.source() + " (" + i.description() + "). " + i.damage() + " dmg.");
//
//            case GameEvent.RoundEnd r -> {
//                if (r.wasted() > 0) put("-- round ends. " + r.wasted() + " AP destroyed unspent.");
//                else put("-- round ends. All AP spent.");
//                r.worldLog().forEach(this::put);
//                dirty = true;
//            }
//            case GameEvent.RoundStart s -> { put("== ROUND " + s.round() + " — AP " + s.ap() + "/" + s.ap()); dirty = true; }
//            case GameEvent.ThrallDied d -> put("*** THE THRALL COLLAPSES. THE LINK GOES DARK. ***");
//        }
//    }
//
//    @Override public void frame(World w, RoundState round) {
//        if (!dirty) { renderLog(); return; }
//        dirty = false;
//        StringBuilder sb = new StringBuilder();
//        sb.append("\n============================ ARCHON — Action Economy Prototype ============================\n");
//        sb.append(grid(w));
//        sb.append(hud(w, round));
//        sb.append("-----------------------------------------------------------------------------------------\n");
//        System.out.print(sb);
//        renderLog();
//    }
...
//                hand/right: %-18s hand/left: %-18s pack %d/%d
//                """,
//                r.roundNo(), t.hp, t.maxHp, pips.toString().trim(), r.ap(), RoundState.BASE_AP,
//                r.linesUsed(), r.taxForNextLine(),
//                name(t.slots.get("hand/right")), name(t.slots.get("hand/left")),
//                t.pack.size(), Thrall.PACK_MAX);
//    }
//
//    private String name(Item i) { return i == null ? "empty" : i.name; }
//}